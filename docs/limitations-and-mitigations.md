...
| 14b | The baseline includes everything the role did in the lookback window, including operator testing | The policy is tightened to observed usage, so test commands run as the demo role (e.g. bucket ACL reads and listing) were treated as legitimate workload usage and widened the next policy from 6 to 8 actions. **Mitigation:** The human dry-run diff review catches this; a production version would separate operator/test sessions from workload sessions in the baseline query. | `Partially mitigated` |
| 14c | Single-user local dashboard access vs centralized team portal | A public live dashboard would require hosted authentication and central credential management, risking data exposure. **Mitigation:** The live dashboard runs strictly on loopback `127.0.0.1` using the developer's local AWS IAM identity with default account/ARN redaction and zero public exposure. Public viewers use the static recorded-run replay on Amplify. | `Mitigated in event window` |
| 14d | DynamoDB Scan for run history listing | `/api/runs` performs a DynamoDB `Scan` sorted in memory rather than querying a Global Secondary Index (GSI) on timestamp. **Mitigation:** Results are bounded by a 72-hour TTL and limit=50. At hackathon and developer scale, this adds negligible overhead and eliminates extra infrastructure complexity. | `Mitigated in event window` |
| 14e | In-flight `PROCESSING` run polling overhead | Live dashboard polls `/api/runs/<uuid>` for runs still in `PROCESSING` state rather than maintaining persistent WebSocket connections. **Mitigation:** Polling is strictly bounded to 60 seconds maximum with 2-second intervals, immediately halts on any terminal state (`COMPLETE`, `BLOCKED`, `ERROR`, etc.) or tab backgrounding, and avoids `setInterval` memory leaks. | `Mitigated in event window` |
